...
console.log(isFoodAmountOk(dogs));

// 7.
const dogsEatingOkAmount = dogs.filter((dog) => isFoodAmountOk(dog));
console.log(dogsEatingOkAmount);

// 8.
// slice to make a shallow copy of an array as to not mutate the original when using sort method
const dogsAccending = dogs.slice().sort((a, b) => {
  if (a.curFood < b.recommendedFood) return -1;
  if (a.curFood > b.recommendedFood) return 1;
  return 0;
});

console.log(dogs);
console.log(dogsAccending);
